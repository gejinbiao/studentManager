...
public class ShiroFactroy implements IShiro {
    ...
    public ShiroUser shiroUser(User user) {
        ...
        for (Integer roleId : roleList) {
            Integer dataAuthority = this.findDataAuthority(roleId);
            if (dataAuthority.equals(1)) {
                shiroUser.setDataAuthority(1);
                break;
            } else {
                shiroUser.setDataAuthority(0);
            }
        }
    ...
    @Override
    public Integer findDataAuthority(Integer roleId) {
        RoleDataAuthority roleDataAuthority = new RoleDataAuthority();
        roleDataAuthority.setRoleId(roleId);
        List<RoleDataAuthority> roleDataAuthoritys = roleDataAuthorityDao.select(roleDataAuthority);
        if (roleDataAuthoritys == null || roleDataAuthoritys.size() == 0) {
            return 0;
        }
        return roleDataAuthoritys.get(0).getDataAuthority();
    }
