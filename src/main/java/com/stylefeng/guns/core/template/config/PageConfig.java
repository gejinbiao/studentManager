...
public class PageConfig {
    ...
    public void init() {
        ...
        codeModels = checkCodeModels(contextConfig.getCodeModels());
    }
     ...
     * @return
     */
    private List<CodeModel> checkCodeModels(List<CodeModel> codeModels) {

        List<CodeModel> list = new ArrayList<>();
        for (CodeModel codeModel : codeModels) {
            if (!codeModel.getAttrName().equals("id") && !codeModel.getAttrName().equals("opreator")) {
                list.add(codeModel);
            }
        }
        return list;
