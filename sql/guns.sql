...
CREATE TABLE `student_infos` (
  ...
  `type` varchar(50) DEFAULT NULL COMMENT '咨询层次',
  `level` char(2) DEFAULT NULL COMMENT '级别(A,B,C,D)',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注',
  `status` char(1) DEFAULT NULL COMMENT '是否报名 0:未报名,1:已报名',
  `next_visit_date` date DEFAULT NULL COMMENT '下次回访日期',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '修改时间',
  `operator` int(11) DEFAULT NULL COMMENT '操作人',
  `current_operator` int(11) DEFAULT NULL COMMENT '当前操作人',
